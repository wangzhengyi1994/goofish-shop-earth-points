...
export class World extends Mini3d {
	constructor(canvas, assets) {
		super(canvas);
		// 中心坐标
		this.geoProjectionCenter = [120.10,29.31];
		// 缩放比例
		this.geoProjectionScale = 120;
		// 飞线中心
		this.flyLineCenter = [120.10,29.31];
		// 地图拉伸高度
		this.depth = 0.5;
		this.mapFocusLabelInfo = {
			name: "浙江省",
			enName: "浙江省 Data visualization",
			center: [120.10,26.81],
		};
		// 是否点击
		this.clicked = true;
	...
	createBar() {
		let self = this;
		let data = sortByValue(provincesData).filter(
			(item, index) => index < 7
		);
		...
		data.map((item, index) => {
			...
			let hg = this.createHUIGUANG(
				geoHeight,
				index > 3 ? 0xfffef4 : 0x77fbf5
			);
			areaBar.add(...hg);
